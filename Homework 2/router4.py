...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    sockTo5 = create_socket(HOST, 8005)
    sockTo6 = create_socket(HOST, 8006)
    ...
    while True:
        ...
        if sending_port[3].strip() == '127.0.0.1':
            print("OUT:", payload)
            write_to_file('./output/out_router_4.txt', new_packet[2])
            
        elif new_ttl:

            if sending_port[3].strip() == '8005':
                print("sending packet", new_packet, "to Router 5")
                sockTo5.send(','.join(new_packet).encode())
                write_to_file('./output/sent_by_router_4.txt', ','.join(new_packet), 5)
                

            elif sending_port[3].strip() == '8006':
                print("sending packet", new_packet, "to Router 6")
                sockTo6.send(','.join(new_packet).encode())
                write_to_file('./output/sent_by_router_4.txt', ','.join(new_packet), 6)
                

            else:
                print("DISCARD:", new_packet)
                write_to_file('./output/discarded_by_router_4.txt', ','.join(new_packet))
